/* m4_nodebug.p4 plus an egress pipeline that records what the queue did.
 *
 * This is the program that shows the ingress / queue / egress split works: it
 * reads standard_metadata.deq_qdepth and deq_timedelta, which have a value only
 * if egress runs after the Days scheduler released the packet. Run it before
 * and after the split and the registers go from all zero to the queue's actual
 * depth and wait. */
...
/* What the queue did, recorded from egress where it is knowable.
 *
 * [0] packets seen in egress          [1] sum of deq_qdepth
 * [2] sum of deq_timedelta (us)       [3] max deq_qdepth
 * [4] packets that saw a non-zero depth
 *
 * Before the pipeline was split these were all zero: egress ran straight after
 * ingress, so the packet had not been in a queue yet. */
register<bit<32>>(8) q;
...
control MyEgress(inout headers hdr, inout metadata meta,
                 inout standard_metadata_t standard_metadata) {
    apply {
        bit<32> n; q.read(n, 32w0); q.write(32w0, n + 1);
        bit<32> d = (bit<32>)standard_metadata.deq_qdepth;
        bit<32> t = (bit<32>)standard_metadata.deq_timedelta;
        bit<32> sd; q.read(sd, 32w1); q.write(32w1, sd + d);
        bit<32> st; q.read(st, 32w2); q.write(32w2, st + t);
        bit<32> mx; q.read(mx, 32w3);
        if (d > mx) { q.write(32w3, d); }
        if (d > 0) { bit<32> nz; q.read(nz, 32w4); q.write(32w4, nz + 1); }
    }
}
