/* Minimal v1model program whose ingress AND egress pipelines both read and
 * write the SAME register array.
 *
...
control MyIngress(inout headers hdr, inout metadata meta,
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            /* P4-level printf: can the user see their own logic run? */
            log_msg("INGRESS dst={} ttl={} counter={}",
                    {hdr.ipv4.dstAddr, hdr.ipv4.ttl, n});
            ipv4_exact.apply();
            log_msg("INGRESS decided egress_spec={}", {standard_metadata.egress_spec});
        }
    }
}
