/* Handoff section 10, Test 1 — passthrough.
 *
 * The smallest program that still proves the integration works end to end:
 * a packet enters, nothing about it changes, and it leaves on the port Days
 * assigned. If this fails, nothing further is worth debugging.
 *
 * Note what it does NOT do: no TTL decrement, no header edit. Test 2 adds that,
 * so a failure there is unambiguous.
 */
#include <core.p4>
#include <v1model.p4>
...
control MyIngress(inout headers hdr, inout metadata meta,
    ...
    apply { if (hdr.ipv4.isValid() && hdr.ports.isValid()) { flow_forward.apply(); } }
}
