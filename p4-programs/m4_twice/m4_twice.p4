/* m4_nodebug.p4 with the table applied twice. The second apply matches the
 * same entry and sets the same egress_spec, so the simulation is identical;
 * the difference in the ingress stage timing is the cost of exactly one
 * table apply, separated from the fixed cost of entering the pipeline. */
